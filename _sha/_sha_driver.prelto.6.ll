; ModuleID = '_sha_driver.prelto.6.bc'
target datalayout = "e-m:e-p:32:32-f64:32:64-f80:32-n8:16:32-S128"
target triple = "i386-unknown-linux-gnu"

@indata = internal unnamed_addr constant [2 x [8192 x i8]] [[8192 x i8] c"", [8192 x i8] c""], align 1
@in_i = internal unnamed_addr constant [2 x i32] [i32 8192, i32 8192], align 4
@sha_info_digest.0 = internal unnamed_addr global i32 0
@sha_info_digest.1 = internal unnamed_addr global i32 0
@sha_info_digest.2 = internal unnamed_addr global i32 0
@sha_info_digest.3 = internal unnamed_addr global i32 0
@sha_info_digest.4 = internal unnamed_addr global i32 0
@sha_info_data = internal unnamed_addr global [16 x i32] zeroinitializer, align 4
@.str = private unnamed_addr constant [11 x i8] c"Result: %d\00", align 1
@.str1 = private unnamed_addr constant [13 x i8] c"RESULT: PASS\00", align 1
@.str2 = private unnamed_addr constant [13 x i8] c"RESULT: FAIL\00", align 1

; Function Attrs: nounwind
define i32 @main() #0 {
  store i32 1732584193, i32* @sha_info_digest.0, align 4, !tbaa !1
  store i32 -271733879, i32* @sha_info_digest.1, align 4, !tbaa !1
  store i32 -1732584194, i32* @sha_info_digest.2, align 4, !tbaa !1
  store i32 271733878, i32* @sha_info_digest.3, align 4, !tbaa !1
  store i32 -1009589776, i32* @sha_info_digest.4, align 4, !tbaa !1
  br label %._crit_edge10.i.i

._crit_edge10.i.i:                                ; preds = %sha_update.exit.i, %0
  %sha_info_count_hi.0 = phi i32 [ 0, %0 ], [ %8, %sha_update.exit.i ]
  %sha_info_count_lo.0 = phi i32 [ 0, %0 ], [ %6, %sha_update.exit.i ]
  %j.02.i = phi i32 [ 0, %0 ], [ %64, %sha_update.exit.i ]
  %1 = getelementptr inbounds [2 x i32]* @in_i, i32 0, i32 %j.02.i
  %2 = load i32* %1, align 4, !tbaa !1
  %3 = getelementptr inbounds [2 x [8192 x i8]]* @indata, i32 0, i32 %j.02.i, i32 0
  %4 = shl i32 %2, 3
  %overflow_intrinsic = zext i32 %sha_info_count_lo.0 to i33
  %overflow_intrinsic1 = zext i32 %4 to i33
  %overflow_intrinsic2 = add i33 %overflow_intrinsic, %overflow_intrinsic1
  %overflow_intrinsic_sum = trunc i33 %overflow_intrinsic2 to i32
  %overflow_intrinsic3 = lshr i33 %overflow_intrinsic2, 32
  %overflow_intrinsic_carry = trunc i33 %overflow_intrinsic3 to i1
  %5 = add i32 %sha_info_count_hi.0, 1
  %.sha_info_count_hi.0 = select i1 %overflow_intrinsic_carry, i32 %5, i32 %sha_info_count_hi.0
  %6 = add i32 %sha_info_count_lo.0, %4
  %7 = lshr i32 %2, 29
  %8 = add i32 %.sha_info_count_hi.0, %7
  %9 = icmp sgt i32 %2, 63
  br i1 %9, label %.preheader.lr.ph.i.i, label %39

.preheader.lr.ph.i.i:                             ; preds = %._crit_edge10.i.i
  %10 = add i32 %2, -64
  %11 = and i32 %10, -64
  br label %.preheader.i.i

.preheader.i.i:                                   ; preds = %memcpy.exit.i.i, %.preheader.lr.ph.i.i
  %.07.i.i = phi i32 [ %2, %.preheader.lr.ph.i.i ], [ %35, %memcpy.exit.i.i ]
  %.016.i.i = phi i8* [ %3, %.preheader.lr.ph.i.i ], [ %34, %memcpy.exit.i.i ]
  br label %12

; <label>:12                                      ; preds = %12, %.preheader.i.i
  %m.03.i.i.i = phi i32 [ %13, %12 ], [ 16, %.preheader.i.i ]
  %p1.02.i.i.i = phi i32* [ %32, %12 ], [ getelementptr inbounds ([16 x i32]* @sha_info_data, i32 0, i32 0), %.preheader.i.i ]
  %p2.01.i.i.i = phi i8* [ %27, %12 ], [ %.016.i.i, %.preheader.i.i ]
  %13 = add nsw i32 %m.03.i.i.i, -1
  %14 = getelementptr inbounds i8* %p2.01.i.i.i, i32 1
  %15 = load i8* %p2.01.i.i.i, align 1, !tbaa !5
  %16 = zext i8 %15 to i32
  %17 = getelementptr inbounds i8* %p2.01.i.i.i, i32 2
  %18 = load i8* %14, align 1, !tbaa !5
  %19 = zext i8 %18 to i32
  %20 = shl nuw nsw i32 %19, 8
  %21 = or i32 %20, %16
  %22 = getelementptr inbounds i8* %p2.01.i.i.i, i32 3
  %23 = load i8* %17, align 1, !tbaa !5
  %24 = zext i8 %23 to i32
  %25 = shl nuw nsw i32 %24, 16
  %26 = or i32 %21, %25
  %27 = getelementptr inbounds i8* %p2.01.i.i.i, i32 4
  %28 = load i8* %22, align 1, !tbaa !5
  %29 = zext i8 %28 to i32
  %30 = shl nuw i32 %29, 24
  %31 = or i32 %26, %30
  store i32 %31, i32* %p1.02.i.i.i, align 4, !tbaa !1
  %32 = getelementptr inbounds i32* %p1.02.i.i.i, i32 1
  %33 = icmp sgt i32 %13, 0
  br i1 %33, label %12, label %memcpy.exit.i.i

memcpy.exit.i.i:                                  ; preds = %12
  tail call fastcc void @sha_transform() #3
  %34 = getelementptr inbounds i8* %.016.i.i, i32 64
  %35 = add nsw i32 %.07.i.i, -64
  %36 = icmp sgt i32 %35, 63
  br i1 %36, label %.preheader.i.i, label %._crit_edge.i.i

._crit_edge.i.i:                                  ; preds = %memcpy.exit.i.i
  %37 = add i32 %11, 64
  %38 = sub i32 %10, %11
  %scevgep.i.i = getelementptr i8* %3, i32 %37
  br label %39

; <label>:39                                      ; preds = %._crit_edge.i.i, %._crit_edge10.i.i
  %.0.lcssa.i.i = phi i32 [ %38, %._crit_edge.i.i ], [ %2, %._crit_edge10.i.i ]
  %.01.lcssa.i.i = phi i8* [ %scevgep.i.i, %._crit_edge.i.i ], [ %3, %._crit_edge10.i.i ]
  %40 = icmp sgt i32 %.0.lcssa.i.i, 3
  br i1 %40, label %.lr.ph.i.i1.i, label %sha_update.exit.i

.lr.ph.i.i1.i:                                    ; preds = %39
  %41 = sdiv i32 %.0.lcssa.i.i, 4
  br label %42

; <label>:42                                      ; preds = %42, %.lr.ph.i.i1.i
  %m.03.i2.i.i = phi i32 [ %41, %.lr.ph.i.i1.i ], [ %43, %42 ]
  %p1.02.i3.i.i = phi i32* [ getelementptr inbounds ([16 x i32]* @sha_info_data, i32 0, i32 0), %.lr.ph.i.i1.i ], [ %62, %42 ]
  %p2.01.i4.i.i = phi i8* [ %.01.lcssa.i.i, %.lr.ph.i.i1.i ], [ %57, %42 ]
  %43 = add nsw i32 %m.03.i2.i.i, -1
  %44 = getelementptr inbounds i8* %p2.01.i4.i.i, i32 1
  %45 = load i8* %p2.01.i4.i.i, align 1, !tbaa !5
  %46 = zext i8 %45 to i32
  %47 = getelementptr inbounds i8* %p2.01.i4.i.i, i32 2
  %48 = load i8* %44, align 1, !tbaa !5
  %49 = zext i8 %48 to i32
  %50 = shl nuw nsw i32 %49, 8
  %51 = or i32 %50, %46
  %52 = getelementptr inbounds i8* %p2.01.i4.i.i, i32 3
  %53 = load i8* %47, align 1, !tbaa !5
  %54 = zext i8 %53 to i32
  %55 = shl nuw nsw i32 %54, 16
  %56 = or i32 %51, %55
  %57 = getelementptr inbounds i8* %p2.01.i4.i.i, i32 4
  %58 = load i8* %52, align 1, !tbaa !5
  %59 = zext i8 %58 to i32
  %60 = shl nuw i32 %59, 24
  %61 = or i32 %56, %60
  store i32 %61, i32* %p1.02.i3.i.i, align 4, !tbaa !1
  %62 = getelementptr inbounds i32* %p1.02.i3.i.i, i32 1
  %63 = icmp sgt i32 %43, 0
  br i1 %63, label %42, label %sha_update.exit.i

sha_update.exit.i:                                ; preds = %42, %39
  %64 = add nsw i32 %j.02.i, 1
  %exitcond.i = icmp eq i32 %64, 2
  br i1 %exitcond.i, label %.lr.ph.i8.preheader.i.i, label %._crit_edge10.i.i

.lr.ph.i8.preheader.i.i:                          ; preds = %sha_update.exit.i
  store i32 128, i32* getelementptr inbounds ([16 x i32]* @sha_info_data, i32 0, i32 0), align 4, !tbaa !1
  %scevgep.i5.i.i = getelementptr [16 x i32]* @sha_info_data, i32 0, i32 1
  %65 = sdiv i32 55, 4
  br label %.lr.ph.i8.i.i

.lr.ph.i8.i.i:                                    ; preds = %.lr.ph.i8.i.i, %.lr.ph.i8.preheader.i.i
  %m.02.i6.i.i = phi i32 [ %66, %.lr.ph.i8.i.i ], [ %65, %.lr.ph.i8.preheader.i.i ]
  %p.11.i7.i.i = phi i32* [ %67, %.lr.ph.i8.i.i ], [ %scevgep.i5.i.i, %.lr.ph.i8.preheader.i.i ]
  %66 = add nsw i32 %m.02.i6.i.i, -1
  %67 = getelementptr inbounds i32* %p.11.i7.i.i, i32 1
  store i32 0, i32* %p.11.i7.i.i, align 4, !tbaa !1
  %68 = icmp sgt i32 %66, 0
  br i1 %68, label %.lr.ph.i8.i.i, label %sha_stream.exit

sha_stream.exit:                                  ; preds = %.lr.ph.i8.i.i
  store i32 %8, i32* getelementptr inbounds ([16 x i32]* @sha_info_data, i32 0, i32 14), align 4, !tbaa !1
  store i32 131072, i32* getelementptr inbounds ([16 x i32]* @sha_info_data, i32 0, i32 15), align 4, !tbaa !1
  tail call fastcc void @sha_transform() #3
  %.pre = load i32* @sha_info_digest.0, align 4, !tbaa !1
  %.pre4 = load i32* @sha_info_digest.1, align 4, !tbaa !1
  %.pre5 = load i32* @sha_info_digest.2, align 4, !tbaa !1
  %.pre6 = load i32* @sha_info_digest.3, align 4, !tbaa !1
  %.pre7 = load i32* @sha_info_digest.4, align 4, !tbaa !1
  %69 = icmp eq i32 %.pre, 6969911
  %70 = zext i1 %69 to i32
  %71 = icmp eq i32 %.pre4, -1814260603
  %72 = zext i1 %71 to i32
  %73 = add nuw nsw i32 %70, %72
  %74 = icmp eq i32 %.pre5, 742465810
  %75 = zext i1 %74 to i32
  %76 = add nuw nsw i32 %73, %75
  %77 = icmp eq i32 %.pre6, 1677179459
  %78 = zext i1 %77 to i32
  %79 = add nuw nsw i32 %76, %78
  %80 = icmp eq i32 %.pre7, -1384908510
  %81 = zext i1 %80 to i32
  %82 = add nuw nsw i32 %79, %81
  %83 = tail call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([11 x i8]* @.str, i32 0, i32 0), i32 %82) #3
  %84 = icmp eq i32 %82, 5
  br i1 %84, label %85, label %87

; <label>:85                                      ; preds = %sha_stream.exit
  %86 = tail call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([13 x i8]* @.str1, i32 0, i32 0)) #3
  br label %89

; <label>:87                                      ; preds = %sha_stream.exit
  %88 = tail call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([13 x i8]* @.str2, i32 0, i32 0)) #3
  br label %89

; <label>:89                                      ; preds = %87, %85
  ret i32 %82
}

; Function Attrs: nounwind
declare i32 @printf(i8* nocapture readonly, ...) #0

; Function Attrs: nounwind
define internal fastcc void @sha_transform() #0 {
  %W = alloca [80 x i32], align 4
  %1 = bitcast [80 x i32]* %W to i8*
  br label %2

; <label>:2                                       ; preds = %2, %0
  %i.029 = phi i32 [ 0, %0 ], [ %6, %2 ]
  %3 = getelementptr inbounds [16 x i32]* @sha_info_data, i32 0, i32 %i.029
  %4 = load i32* %3, align 4, !tbaa !1
  %5 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %i.029
  store i32 %4, i32* %5, align 4, !tbaa !1
  %6 = add nsw i32 %i.029, 1
  %exitcond41 = icmp eq i32 %6, 16
  br i1 %exitcond41, label %.preheader3, label %2

.preheader3:                                      ; preds = %.preheader3, %2
  %i.128 = phi i32 [ %23, %.preheader3 ], [ 16, %2 ]
  %7 = add nsw i32 %i.128, -3
  %8 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %7
  %9 = load i32* %8, align 4, !tbaa !1
  %10 = add nsw i32 %i.128, -8
  %11 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %10
  %12 = load i32* %11, align 4, !tbaa !1
  %13 = xor i32 %12, %9
  %14 = add nsw i32 %i.128, -14
  %15 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %14
  %16 = load i32* %15, align 4, !tbaa !1
  %17 = xor i32 %13, %16
  %18 = add nsw i32 %i.128, -16
  %19 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %18
  %20 = load i32* %19, align 4, !tbaa !1
  %21 = xor i32 %17, %20
  %22 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %i.128
  store i32 %21, i32* %22, align 4, !tbaa !1
  %23 = add nsw i32 %i.128, 1
  %exitcond40 = icmp eq i32 %23, 80
  br i1 %exitcond40, label %24, label %.preheader3

; <label>:24                                      ; preds = %.preheader3
  %25 = load i32* @sha_info_digest.0, align 4, !tbaa !1
  %26 = load i32* @sha_info_digest.1, align 4, !tbaa !1
  %27 = load i32* @sha_info_digest.2, align 4, !tbaa !1
  %28 = load i32* @sha_info_digest.3, align 4, !tbaa !1
  %29 = load i32* @sha_info_digest.4, align 4, !tbaa !1
  br label %30

; <label>:30                                      ; preds = %30, %24
  %E.027 = phi i32 [ %29, %24 ], [ %D.026, %30 ]
  %D.026 = phi i32 [ %28, %24 ], [ %C.025, %30 ]
  %C.025 = phi i32 [ %27, %24 ], [ %46, %30 ]
  %B.024 = phi i32 [ %26, %24 ], [ %A.023, %30 ]
  %A.023 = phi i32 [ %25, %24 ], [ %43, %30 ]
  %i.222 = phi i32 [ 0, %24 ], [ %47, %30 ]
  %31 = shl i32 %A.023, 5
  %32 = lshr i32 %A.023, 27
  %33 = or i32 %31, %32
  %34 = and i32 %C.025, %B.024
  %35 = xor i32 %B.024, -1
  %36 = and i32 %D.026, %35
  %37 = or i32 %36, %34
  %38 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %i.222
  %39 = load i32* %38, align 4, !tbaa !1
  %40 = add i32 %33, 1518500249
  %41 = add i32 %40, %E.027
  %42 = add i32 %41, %37
  %43 = add i32 %42, %39
  %44 = shl i32 %B.024, 30
  %45 = lshr i32 %B.024, 2
  %46 = or i32 %44, %45
  %47 = add nsw i32 %i.222, 1
  %exitcond39 = icmp eq i32 %47, 20
  br i1 %exitcond39, label %.preheader2, label %30

.preheader2:                                      ; preds = %.preheader2, %30
  %E.121 = phi i32 [ %D.120, %.preheader2 ], [ %D.026, %30 ]
  %D.120 = phi i32 [ %C.119, %.preheader2 ], [ %C.025, %30 ]
  %C.119 = phi i32 [ %61, %.preheader2 ], [ %46, %30 ]
  %B.118 = phi i32 [ %A.117, %.preheader2 ], [ %A.023, %30 ]
  %A.117 = phi i32 [ %58, %.preheader2 ], [ %43, %30 ]
  %i.316 = phi i32 [ %62, %.preheader2 ], [ 20, %30 ]
  %48 = shl i32 %A.117, 5
  %49 = lshr i32 %A.117, 27
  %50 = or i32 %48, %49
  %51 = xor i32 %C.119, %B.118
  %52 = xor i32 %51, %D.120
  %53 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %i.316
  %54 = load i32* %53, align 4, !tbaa !1
  %55 = add i32 %50, 1859775393
  %56 = add i32 %55, %52
  %57 = add i32 %56, %E.121
  %58 = add i32 %57, %54
  %59 = shl i32 %B.118, 30
  %60 = lshr i32 %B.118, 2
  %61 = or i32 %59, %60
  %62 = add nsw i32 %i.316, 1
  %exitcond38 = icmp eq i32 %62, 40
  br i1 %exitcond38, label %.preheader1, label %.preheader2

.preheader1:                                      ; preds = %.preheader1, %.preheader2
  %E.215 = phi i32 [ %D.214, %.preheader1 ], [ %D.120, %.preheader2 ]
  %D.214 = phi i32 [ %C.213, %.preheader1 ], [ %C.119, %.preheader2 ]
  %C.213 = phi i32 [ %78, %.preheader1 ], [ %61, %.preheader2 ]
  %B.212 = phi i32 [ %A.211, %.preheader1 ], [ %A.117, %.preheader2 ]
  %A.211 = phi i32 [ %75, %.preheader1 ], [ %58, %.preheader2 ]
  %i.410 = phi i32 [ %79, %.preheader1 ], [ 40, %.preheader2 ]
  %63 = shl i32 %A.211, 5
  %64 = lshr i32 %A.211, 27
  %65 = or i32 %63, %64
  %66 = or i32 %D.214, %C.213
  %67 = and i32 %66, %B.212
  %68 = and i32 %D.214, %C.213
  %69 = or i32 %67, %68
  %70 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %i.410
  %71 = load i32* %70, align 4, !tbaa !1
  %72 = add i32 %65, -1894007588
  %73 = add i32 %72, %E.215
  %74 = add i32 %73, %69
  %75 = add i32 %74, %71
  %76 = shl i32 %B.212, 30
  %77 = lshr i32 %B.212, 2
  %78 = or i32 %76, %77
  %79 = add nsw i32 %i.410, 1
  %exitcond37 = icmp eq i32 %79, 60
  br i1 %exitcond37, label %.preheader, label %.preheader1

.preheader:                                       ; preds = %.preheader, %.preheader1
  %E.39 = phi i32 [ %D.38, %.preheader ], [ %D.214, %.preheader1 ]
  %D.38 = phi i32 [ %C.37, %.preheader ], [ %C.213, %.preheader1 ]
  %C.37 = phi i32 [ %93, %.preheader ], [ %78, %.preheader1 ]
  %B.36 = phi i32 [ %A.35, %.preheader ], [ %A.211, %.preheader1 ]
  %A.35 = phi i32 [ %90, %.preheader ], [ %75, %.preheader1 ]
  %i.54 = phi i32 [ %94, %.preheader ], [ 60, %.preheader1 ]
  %80 = shl i32 %A.35, 5
  %81 = lshr i32 %A.35, 27
  %82 = or i32 %80, %81
  %83 = xor i32 %C.37, %B.36
  %84 = xor i32 %83, %D.38
  %85 = getelementptr inbounds [80 x i32]* %W, i32 0, i32 %i.54
  %86 = load i32* %85, align 4, !tbaa !1
  %87 = add i32 %82, -899497514
  %88 = add i32 %87, %84
  %89 = add i32 %88, %E.39
  %90 = add i32 %89, %86
  %91 = shl i32 %B.36, 30
  %92 = lshr i32 %B.36, 2
  %93 = or i32 %91, %92
  %94 = add nsw i32 %i.54, 1
  %exitcond = icmp eq i32 %94, 80
  br i1 %exitcond, label %95, label %.preheader

; <label>:95                                      ; preds = %.preheader
  %.lcssa7 = phi i32 [ %93, %.preheader ]
  %.lcssa = phi i32 [ %90, %.preheader ]
  %A.35.lcssa = phi i32 [ %A.35, %.preheader ]
  %C.37.lcssa = phi i32 [ %C.37, %.preheader ]
  %D.38.lcssa = phi i32 [ %D.38, %.preheader ]
  %96 = add i32 %25, %.lcssa
  store i32 %96, i32* @sha_info_digest.0, align 4, !tbaa !1
  %97 = add i32 %26, %A.35.lcssa
  store i32 %97, i32* @sha_info_digest.1, align 4, !tbaa !1
  %98 = add i32 %27, %.lcssa7
  store i32 %98, i32* @sha_info_digest.2, align 4, !tbaa !1
  %99 = add i32 %28, %C.37.lcssa
  store i32 %99, i32* @sha_info_digest.3, align 4, !tbaa !1
  %100 = add i32 %29, %D.38.lcssa
  store i32 %100, i32* @sha_info_digest.4, align 4, !tbaa !1
  ret void
}

; Function Attrs: nounwind
declare void @llvm.lifetime.start(i64, i8* nocapture) #1

; Function Attrs: nounwind
declare void @llvm.lifetime.end(i64, i8* nocapture) #1

; Function Attrs: nounwind readnone
declare { i32, i1 } @llvm.uadd.with.overflow.i32(i32, i32) #2

attributes #0 = { nounwind "less-precise-fpmad"="false" "no-frame-pointer-elim"="false" "no-infs-fp-math"="false" "no-nans-fp-math"="false" "stack-protector-buffer-size"="8" "unsafe-fp-math"="false" "use-soft-float"="false" }
attributes #1 = { nounwind }
attributes #2 = { nounwind readnone }
attributes #3 = { nobuiltin nounwind }

!llvm.ident = !{!0}

!0 = metadata !{metadata !"clang version 3.5.0 (tags/RELEASE_350/final)"}
!1 = metadata !{metadata !2, metadata !2, i64 0}
!2 = metadata !{metadata !"int", metadata !3, i64 0}
!3 = metadata !{metadata !"omnipotent char", metadata !4, i64 0}
!4 = metadata !{metadata !"Simple C/C++ TBAA"}
!5 = metadata !{metadata !3, metadata !3, i64 0}
